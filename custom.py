...
class VisionPonder(nn.Module):
    def __init__(self, n_elems: int, n_hidden: int, max_steps: int, n_classes: int ):
        super().__init__()
        ...
        self.output_layer = nn.Linear(768, n_classes)
        self.lambda_layer = nn.Linear(768, 1)
...
class ReconstructionLoss(nn.Module):
    ...
    def forward(self, p: torch.Tensor, y_hat: torch.Tensor, y:torch.Tensor):
        ...
        for n in range(p.shape[0]):
            loss = (p[n] * self.loss_func(y_hat[n], torch.tensor(y,dtype=torch.long))).mean()
